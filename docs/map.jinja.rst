...
We will see a quick setup to configure the ``warden`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``warden`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``warden`` formulas under ``/srv/salt/warden/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/warden/parameters/dns:domain/
    ln -s dns:domain /srv/salt/warden/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/warden/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/warden/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``warden`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``warden`` formulas under ``/srv/salt/warden/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/warden/parameters/roles
...
We create a configuration for the role ``warden/server`` in ``/srv/salt/warden/parameters/roles/warden/server.yaml``:
...
We create another configuration for the role ``warden/client`` in ``/srv/salt/warden/parameters/roles/warden/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles warden/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles warden/client
...
.. code-block:: console
    ...
    mv /srv/salt/warden/parameters/dns:domain/  '/srv/salt/warden/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-warden-as-json:
      test.nop:
        - name: {{ warden | json }}
...
Here is an example based on `template-formula/warden/config/file.sls`_:
...
This ``sls`` file expose a ``warden`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ warden['winner'] }}
...
.. _template-formula/warden/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/warden/config/file.sls
